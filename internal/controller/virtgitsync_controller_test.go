		...
		It("should successfully reconcile and add finalizer", func() {
			...
			resource := &virtv1alpha1.VirtGitSync{
				...
				Spec: virtv1alpha1.VirtGitSyncSpec{
					GitRepository: virtv1alpha1.GitRepositorySpec{
			...
			// Note: We don't check status here because the git clone will fail
			// (test repository doesn't exist). The important thing is the finalizer is added.
		})
	})
